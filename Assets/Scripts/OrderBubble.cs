...
public class OrderBubble : MonoBehaviour
{
    ...
    // Private method to calculate the position for a given slot index.
    ...
    //     // Auto-calculate slotSpacing if it is set to zero.

    //     float orderBubbleHeight = selfSpriteRenderer.bounds.size.y;
    //     float foodHeight = food.spriteRenderer.bounds.size.y;
    //     slotSpacing = (orderBubbleHeight - (foodHeight * maxSlots)) / maxSlots;


    //     // Calculate vertical offset based on slot index.
    //     float midSlot = (maxSlots - 1) / 2.0f;
    //     float yOffset = slotSpacing * (midSlot - slotIndex);
    ...
    private Vector3 CalculateOrderPosition(int slotIndex)
    {
        ...
        float[] percentages = new float[3] { 0.70f, 0.40f, 0.10f };
